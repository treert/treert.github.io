 ...
 *
 * `validateEndgameFen` 会拒绝「轮走方被将军」和「已经终局」的局面 ——
 * 那是**出题**的要求（拿它当练习起点，不该一上来就被将）。
 *
 * 但分享是**对局中途**截出来的：你刚走了一步将军，这时复制链接，
 * 接收方打开就该看到那个被将的局面。所以这里只查 `isLegalPosition`
 ...
 * 当前局面的分享链接。
 *
 * **先清空已有的查询串**：那可能是上一次分享留下的 fen，也可能是别的参数
 * （比如调试时加的 `?v=`）。分享链接应当只带这一个参数，否则会带出旧局面的残渣。
 *
 * **空格编成 `%20`，不用 `searchParams.set`。** 后者会把空格编成 `+`，
 * 而 `+` 在查询串里是否等于空格**取决于解析方** —— HTML 表单场景下是，
 * 别的地方不一定。FEN 里有 5 个空格，一旦有一处把 `+` 当成字面量，
 * 整条 FEN 就废了。`%20` 没有这个歧义。
 *
