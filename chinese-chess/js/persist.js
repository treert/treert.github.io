...
export const STORAGE_KEY = 'chinese-chess:state';
...
function isValid(data) {
  ...
  for (const m of data.moves) {
    ...
    if (!Number.isInteger(m.move) || m.move < 0 || m.move >= 8100) return false;
    if (typeof m.notation !== 'string') return false;
...
export function restoreInto(game, storage = defaultStorage()) {
  ...
  game.followBook = !!data.followBook; // 同理：老存档没有 → false（默认关）
  return true;
}
