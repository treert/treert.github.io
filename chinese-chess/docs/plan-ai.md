...
// --- 搜索：negamax + alpha-beta ---
{
  ...
  {
    const fen = '5k3/9/9/9/9/9/9/3n5/9/3KR4 w - - 0 1';
  ...
  {
    const fen = '4k4/3R5/9/9/9/9/9/9/9/5K3 b - - 0 1';
    const r = search(fen, plain, { rng: seededRng(1) });
...
> | 用途 | FEN | 子力（坐标 `x,y`） |
> |------|-----|---------------------|
> | 一步将死 | `3k5/9/9/9/9/4R4/9/9/9/R3K4 w - - 0 1` | 黑将 `3,0`；红车 `4,5`、`0,9`；红帅 `4,9` |
> | 吃白送的子 | `5k3/9/9/9/4r4/9/9/9/9/3KR4 w - - 0 1` | 黑将 `5,0`；黑车 `4,4`；红帅 `3,9`；红车 `4,9` |
> | 救子 | `5k3/9/9/9/9/9/9/3n5/9/3KR4 w - - 0 1` | 黑将 `5,0`；黑马 `3,7`；红帅 `3,9`；红车 `4,9` |
> | 唯一着法 | `4k4/3R5/9/9/9/9/9/9/9/5K3 b - - 0 1` | 黑将 `4,0`；红车 `3,1`；红帅 `5,9` |
...
>
> **所有局面都满足「两将不在同一条纵线上」**（除了「已经终局」那局，它的黑将正被将军，是刻意的）。
> 写新局面时务必单独确认这一条 —— 上一轮实现规则层时，就是因为把黑将放在了红帅同一条纵线上，
> 把「没被将军」测成了照面，而且另一条断言**碰巧绿了**、根本没测到想测的东西。
>
> 另外注意「唯一着法」那局：不能图省事写成「红帅 `4,9` + 黑将 `4,0`」，那是照面，
> 黑方反而有两个合法着法，测不到「唯一着法」。
...
// --- 挡位弱化 ---
{
  ...
  {
    const blunder = { ...strong, blunderRate: 1 };
    const fen1 = '4k4/3R5/9/9/9/9/9/9/9/5K3 b - - 0 1';
