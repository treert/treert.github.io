...
**如果「红方共 44 步」失败，先看上面那 7 条分解断言哪条挂了**——分解断言就是为了定位这个用的，不要直接去改 44 这个数字。

44 是这样推出来的：5 兵 × 1 + 2 炮 × 12 + 2 马 × 2 + 2 车 × 2 + 2 相 × 2 + 2 仕 × 1 + 1 帅 × 1。

**炮的 12 步是最容易数错的一项**，它有两个陷阱：

1. 红炮在 `(1,7)` 时，除了向上 4 步、向左 1 步、向右 5 步，还能**向下走到 `(1,8)`**（`(1,8)` 是空的，再往下的 `(1,9)` 是自家马）。
2. 向上走时，`(1,2)` 的黑炮挡路——但**炮架的阻挡作用不是终点**：黑炮本身就是炮架，它后面的 `(1,0)` 是黑马，所以这一步**是合法的吃子**。数「炮能走几步」时，容易只数到炮架为止。

第 2 条是实际的踩坑记录：这一版的初稿把炮数成 11、总数写成 42，就是漏了这次吃子。
...
**7. 初始局面着法数是 44，不是 42。** 本计划初稿（以及 `design.md` §12.1 的初稿）把这个数字写成 42，理由是「炮各 11 步」。实际执行到 Task 3 时，分解断言精确指出是**炮**这一项错了：红炮在 `(1,7)` 向上走时，`(1,2)` 的黑炮是炮架，而炮架后面的 `(1,0)` 是黑马——**这一步是合法的吃子**。数「炮能走几步」时只数到炮架为止，是很容易犯的错。

正确分解：5 兵 × 1 + **2 炮 × 12** + 2 马 × 2 + 2 车 × 2 + 2 相 × 2 + 2 仕 × 1 + 1 帅 × 1 = **44**。

这件事本身就是「把总数拆到每个棋子上」这个测试设计的价值证明：如果只断言总数 42，失败信息只会告诉你「不对」，然后得从头手推一遍；有了分解断言，一眼就定位到炮。**后面几层如果还要加「初始局面着法数」这类断言，照这个方式拆。**

**8. Task 4 的 `inCheck` 测试局面是错的：黑将不能放在红帅同一条纵线上。** 初稿写成 `build(['K@4,9', 'r@3,0', 'k@4,0'])` 并期望「没被将军」，但两将同处纵线 4 且中间全空——**这本身就是照面，等于被将军**，实现返回 `true` 是对的。

同一个坑还有一处更隐蔽的：初稿的「被将军时 inCheck 为真」写成 `build(['K@4,9', 'r@4,0', 'k@4,1'])`，它虽然断言通过了，但**通过的原因是照面而不是车**——测试碰巧绿了，实际没测到想测的东西。两条都改成把黑将放在 `(5,0)`。

**结论：写涉及将 / 帅的局面时，必须单独确认两将不在同一纵线上，或中间确实有子。** 顺手补了一条 `只有将帅照面、没有别的攻击子时也算被将军`，把这个行为正面钉住。

**9. Task 4 的两处「合法着法」期望值也要算上照面。** 「帅只能躲到两侧」那条，`(3,9)` 会与 `(3,0)` 的黑将照面，所以合法落点比直觉少一个。这类断言不能凭「哪格空着」推，必须把照面一起算进去——好在 `generateLegalMoves` 已经正确处理，只是手写的期望值要对得上。
