...
**把 24 局 `repeat` / `cap` 全部用 5 秒/手重跑，又出线 8 局**（约 38 分钟，2026-09-21）：

```powershell
node chinese-chess/tools/solve.mjs --ids <24 个 id> --movetime 5000 --rules 40 --no-slow
# → 004(32 步杀) 157(29) 278(31) 302(33) 342(24) 349(23，被 go mate 直接证明) 419(20) 422(45)
#   剩 15 局仍 repeat、1 局掉到 notWinning。解决方案总数 431 → 439（398 已证明 + 41 参考线）
```

**命中率 8/24 ≈ 33%** —— 所以「1.5 秒/手跑不出来」里混着相当比例的「预算不够」，
读 `repeat` 时别当成"这局没救"的定论。代价是线性涨：这批 24 局跑了 38 分钟。
