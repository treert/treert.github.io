/**
 * 自定义局面：用户自己存的局面。纯逻辑 —— storage 通过参数注入（与 persist.js 同一个路子）。
 *
 ...
 *
 * 两者在 endgames.js 里通过注册表合并成同一个查询入口，
 * 所以 game.js / persist.js **完全不需要知道有「自定义」这回事**。
 *
 * ## 为什么自定义局面一律没有 result
 *
 * 静态库每局都标「先手胜 / 和棋」，界面靠它显示「谱载先手胜」并判定「达成目标」。
 * 但用户存的是对局中途或自己摆的局面，程序无从知道胜负。硬给一个「先手胜」就是编造事实。
 * 所以自定义局面 result 恒为 null，界面显示「自定义局面」、不做达成判定。
 *
 * ## 校验失败一律带原因
 *
 * 用户是手粘 FEN 的，说「不行」而不说为什么等于没说。校验函数都返回 `{ ok, reason }`。
 * 分两个层次：
 *
...
import { CUSTOM_CATEGORY, CUSTOM_SOURCE } from './endgames.js';

export const CUSTOM_KEY = 'chess:endgames';
export { CUSTOM_CATEGORY, CUSTOM_SOURCE };
 ...
 *
 * `fen` 是**规范化后**的：尾部三个字段统一、空白去掉。规范化有两个好处 ——
 * 存进去的和读出来的永远一致（往返可比），以及「同一个局面粘两次」能被去重认出来。
 *
 * 这是「**自由局面**」的准入标准：粘一段 FEN 直接摆到棋盘上走它（见 main.js 的 loadFen）。
 * `share.js` 的 `readShareFen` 是同一套标准，只是它的错误文案带「链接里的」。
 */
 ...
 *
 * 比 `validateFreeFen` 只多一条：轮走方得有合法着法 —— 已经终局（将死 / 逼和）的局面
 * 存下来也练不了。
 *
 * **「轮走方已经被将军」不禁**：那是合法局面（他应将就是了），
 * 而「我正被将、该怎么解」是正当需求。真正非法的「非轮走方被将军」由 isLegalPosition 挡住。
 */
 ...
 * 读出全部自定义局面，新的在前。
 *
 * **逐条校验**：坏的那条丢掉、好的留下。这和 persist.js 的「整份丢弃」正好相反，
 * 因为语义不同 —— 那边是「一个对局」，这边是「一个列表」（丢一条好过丢全部）。
 */
