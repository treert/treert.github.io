...
- 棋子：内联 SVG（6 种形状各一个 `<symbol>`，`fill: currentColor` + `stroke` 走变量），
...
- [ ] `node chess/tools/test-rules.mjs`（含 perft 1~5 + Kiwipete 等四个局面）退出码 0
- [ ] `node chess/tools/test-notation.mjs` / `test-game.mjs` / `test-engine.mjs` / `test-share.mjs` 退出码 0
- [ ] `node chess/tools/verify-endgames.mjs` 退出码 0
- [ ] 人机对弈能下完一盘（含升变、吃过路兵、易位各至少出现一次）
- [ ] 悔棋 / 重做 / 单步回看 / 翻转 / 提示 全部可用，动画符合 `design.md` §3.8
- [ ] 深色模式配色正常；窄屏单列布局正常
- [ ] 刷新不丢局面；分享链接能复现局面
- [ ] 首页卡片与两份仓库文档都已更新
