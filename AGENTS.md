...
`tmp/` 是**临时文件和工具的堆放处**，已经在 `.gitignore` 里，所以：

- 里面的东西不会被提交，也不会被 GitHub Pages 发布——放进去的东西等于只存在于本地。
- 可以随手往里丢：调试脚本、探测数据（json / log）、下载的第三方二进制、实验性输出……
- **可以随时整个删掉**，不需要征求同意，也不要往里面放任何需要长期保留的东西。
- 反过来，`tmp/` 里的文件**不能**被首页或其他模块引用（`<script src>` / `<link href>` / fetch）——
  发布后它们不存在，线上必然 404。
- 看到里面一堆不认识、甚至已经过期的文件和目录（比如某个引擎的探测报告），是正常的。

要写一次性的脚本或中间产物就直接放 `tmp/`，不要污染仓库根目录和各模块目录。
